...
//枚举有两种：1.enum：不限定作用域 2.enum class :限定作用域（可以避免重定义） 参考文章链接：https://www.yisu.com/zixun/311515.html
enum class SensorType { VELODYNE, OUSTER, LIVOX };  //枚举类型，且限定作用域          enum class 枚举类型{枚举成员1，枚举成员2，...}
...
public:
    ...
    ParamServer()
    {
        nh.param<std::string>("/robot_id", robot_id, "roboat");  //获取''/robbot_id"的值给robot_id,如果没有，则使用默认值
    ...
    */
    sensor_msgs::Imu imuConverter(const sensor_msgs::Imu& imu_in)
    {
        ...
        Eigen::Quaterniond q_final = q_from * extQRPY;  //坐标系变换是右乘四元数的逆！！！！！
...
sensor_msgs::PointCloud2 publishCloud(const ros::Publisher& thisPub, const T& thisCloud, ros::Time thisStamp, std::string thisFrame)
{
    ...
    pcl::toROSMsg(*thisCloud, tempCloud);   //pcl点云转换成ROS的消息格式
