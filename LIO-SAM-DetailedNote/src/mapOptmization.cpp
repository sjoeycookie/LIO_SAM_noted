...
public:
    ...
    */
    mapOptimization()
    {
        ...
        downSizeFilterCorner.setLeafSize(mappingCornerLeafSize, mappingCornerLeafSize, mappingCornerLeafSize);  //0.2*0.2*0.2
        downSizeFilterSurf.setLeafSize(mappingSurfLeafSize, mappingSurfLeafSize, mappingSurfLeafSize);  //0.4*0.4*0.4
        downSizeFilterICP.setLeafSize(mappingSurfLeafSize, mappingSurfLeafSize, mappingSurfLeafSize);     //0.4*0.4*0.4
        downSizeFilterSurroundingKeyPoses.setLeafSize(surroundingKeyframeDensity, surroundingKeyframeDensity, surroundingKeyframeDensity); // for surrounding key poses of scan-to-map optimization(2.0*2.0*2.0)
    ...
    */
     ...
     * 8、发布里程计、点云、轨迹
    */
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr& msgIn)   //!核心
    {
        // 当前激光帧时间戳
    ...
    */
    pcl::PointCloud<PointType>::Ptr transformPointCloud(pcl::PointCloud<PointType>::Ptr cloudIn, PointTypePose* transformIn)
    {
        ...
        // 使用openmp进行并行加速 
        #pragma omp parallel for num_threads(numberOfCores)   //4个线程
    ...
    */
    void publishGlobalMap()
    {
        ...
        kdtreeGlobalMap->radiusSearch(cloudKeyPoses3D->back(), globalMapVisualizationSearchRadius, pointSearchIndGlobalMap, pointSearchSqDisGlobalMap, 0); //*1000m(半径)
        mtx.unlock();
        ...
        downSizeFilterGlobalMapKeyPoses.setLeafSize(globalMapVisualizationPoseDensity, globalMapVisualizationPoseDensity, globalMapVisualizationPoseDensity); // 网格大小10*10*10(for global map visualization)
        ...
        downSizeFilterGlobalMapKeyFrames.setLeafSize(globalMapVisualizationLeafSize, globalMapVisualizationLeafSize, globalMapVisualizationLeafSize); //网格大小1.0*1.0*1.0  (for global map visualization)
    ...
    */
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        icp.setMaxCorrespondenceDistance(historyKeyframeSearchRadius*2);   //15*2 =30
        ...
        // 添加闭环因子需要的数据
        mtx.lock();
        loopIndexQueue.push_back(make_pair(loopKeyCur, loopKeyPre));    //当前帧和闭环候选帧的索引
        loopPoseQueue.push_back(poseFrom.between(poseTo));   //当前帧与候选帧之间的位姿
        loopNoiseQueue.push_back(constraintNoise);              //噪声模型
        mtx.unlock();

        loopIndexContainer[loopKeyCur] = loopKeyPre;  // 保存已存在的约束对
    }
    ...
    */
    bool detectLoopClosureDistance(int *latestID, int *closestID)
    {
        ...
        for (int i = 0; i < (int)pointSearchIndLoop.size(); ++i)
        {
            ...
            if (abs(copy_cloudKeyPoses6D->points[id].time - timeLaserInfoCur) > historyKeyframeSearchTimeDiff)   //30s
            {
                loopKeyPre = id;
                break;
            }
    ...
    */
    void loopFindNearKeyframes(pcl::PointCloud<PointType>::Ptr& nearKeyframes, const int& key, const int& searchNum)
    {
        ...
        nearKeyframes->clear();
        int cloudSize = copy_cloudKeyPoses6D->size();
        for (int i = -searchNum; i <= searchNum; ++i)    //通过-searchNum 到 +searchNum，搜索key两侧内容
        {
            int keyNear = key + i;
            if (keyNear < 0 || keyNear >= cloudSize )  // 如果超出范围就算了
                continue;
    ...
    */
    void visualizeLoopClosure()
    {
        ...
        pubLoopConstraintEdge.publish(markerArray);  // 最后发布出去供可视化使用
    }
    ...
    */
    void extractNearby()
    {
        ...
        kdtreeSurroundingKeyPoses->radiusSearch(cloudKeyPoses3D->back(), (double)surroundingKeyframeSearchRadius, pointSearchInd, pointSearchSqDis);  //半径50m
    ...
    */
    void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract)
    {
        ...
        for (int i = 0; i < (int)cloudToExtract->size(); ++i)
        {
            ...
            if (laserCloudMapContainer.find(thisKeyInd) != laserCloudMapContainer.end()) //* 如果这个关键帧对应的点云信息已经存储在一个地图容器里
            {
    ...
    */
    void cornerOptimization()
    {
        ...
        // 遍历当前帧角点集合
        #pragma omp parallel for num_threads(numberOfCores)  //使用openmp并行加速
    ...
    */
    ...
    /**
     * 用imu原始RPY数据与scan-to-map优化后的位姿进行加权融合，更新当前帧位姿的roll、pitch，约束z坐标 (//todo 抑制地图漂移)
    */
    ...
    */
    void addGPSFactor()
    {
        ...
        // 位姿协方差很小，没必要加入GPS数据进行校正   //?3和4我猜可能是x和y？（6维，roll，pitch，yaw，x，y，z）
        ...
        while (!gpsQueue.empty())
        {
            ...
            else
            {
                ...
                aLoopIsClosed = true;  // 加入gps之后等同于回环，需要触发较多的isam update
                break;
            }
        }
    }
    ...
    */
    void addLoopFactor() 
    {
        if (loopIndexQueue.empty())   //当前帧和闭环候选帧的索引
            return;
        ...
        {
            // 闭环边对应两帧的索引
            int indexFrom = loopIndexQueue[i].first;         //当前关键帧索引
            int indexTo = loopIndexQueue[i].second;         //闭环候选关键帧索引
            // 闭环边的位姿变换
            gtsam::Pose3 poseBetween = loopPoseQueue[i];   //当前关键帧与候选关键帧之间的位姿
            gtsam::noiseModel::Diagonal::shared_ptr noiseBetween = loopNoiseQueue[i];   //噪声模型
    ...
    */
    void saveKeyFramesAndFactor()
    {
        ...
        if (aLoopIsClosed == true)  // 如果加入了gps的约束或者回环约束，isam需要进行更多次的优化
        {
            isam->update();
            isam->update();
            isam->update();
    ...
    */
    void updatePath(const PointTypePose& pose_in)
    {
        ...
        pose_stamped.header.frame_id = odometryFrame;   //odom坐标系
    ...
    */
    void publishOdometry()
    {
        ...
        tf::StampedTransform trans_odom_to_lidar = tf::StampedTransform(t_odom_to_lidar, timeLaserInfoStamp, odometryFrame, "lidar_link");    //父坐标系:odom ,子坐标系：lidar_link
        ...
        if (lastIncreOdomPubFlag == false)   //第一次数据直接用全局里程计初始化
        {
            ...
            //todo 用imu原始RPY数据与图优化后的里程计增量位姿进行加权融合，更新当前帧位姿的roll、pitch
            if (cloudInfo.imuAvailable == true)
            {
                if (std::abs(cloudInfo.imuPitchInit) < 1.4)
                {
            ...
            if (isDegenerate)
                laserOdomIncremental.pose.covariance[0] = 1;   //如果退化，位姿协方差设为1
            else 
                laserOdomIncremental.pose.covariance[0] = 0;     //不退化，设为0
        }
