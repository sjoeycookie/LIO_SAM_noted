...
Date: 2021-02-21 
--------------------------------------------------
TransformFusion类 （基于全局信息的IMU里程计)
功能简介：
...
--------------------------------------------------
IMUPreintegration类  (基于增量信息的IMU里程计)
功能简介：
...
public:
    ...
    */
    TransformFusion()
    {
        ...
        if(lidarFrame != baselinkFrame)           //todo 代码未执行
        {
            try
            {
                // 等待3s
                tfListener.waitForTransform(lidarFrame, baselinkFrame, ros::Time(0), ros::Duration(3.0));
                // baselink系相对于lidar系的变换关系
    ...
    */
    void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        if(lidarFrame != baselinkFrame)           
            tCur = tCur * lidar2Baselink;              //todo     T_w_base   = T_w_l * T_l_base
        tf::StampedTransform odom_2_baselink = tf::StampedTransform(tCur, odomMsg->header.stamp, odometryFrame, baselinkFrame);      //发布baselink相对于odom系的坐标变换（note:odom和map同一个系)
...
public:
    ...
    // imu-lidar位姿变换
    gtsam::Pose3 imu2Lidar = gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(-extTrans.x(), -extTrans.y(), -extTrans.z()));  //T_b'_l  (雷达--->中间系)
    gtsam::Pose3 lidar2Imu = gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(extTrans.x(), extTrans.y(), extTrans.z()));  //T_l_b'  (中间系--->雷达)
    ...
    */
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        if (systemInitialized == false)
        {
            ...
            // 添加里程计位姿先验因子
            prevPose_ = lidarPose.compose(lidar2Imu);   //todo T_w_b' = T_w_l * T_l_b'
        ...
        // 添加位姿因子
        gtsam::Pose3 curPose = lidarPose.compose(lidar2Imu);   //todo T_w_b' = T_w_l * T_l_b'
        ...
        prevStateOdom = prevState_;  //用于在imu回调函数中计算预积分量
    ...
    */
    bool failureDetection(const gtsam::Vector3& velCur, const gtsam::imuBias::ConstantBias& biasCur)
    {
        ...
        if (vel.norm() > 30)   //norm返回的是向量的二范数，即模长  参考文章链接:https://blog.csdn.net/xhtchina/article/details/118656026
        {
    ...
    */
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw)
    {
        ...
        if (doneFirstOpt == false)   //该值在激光里程计的回调函数设置(如果因子图优化成功，则设置为true)
            return;
        ...
        gtsam::Pose3 lidarPose = imuPose.compose(imu2Lidar);   //T_w_l = T_w_b* x T_b*_l
